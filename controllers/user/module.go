...
var LoginQuery string = `SELECT users.id as ID,users.name as Name,users.lastname as LastName, COALESCE(users.languageid,0) as languageid, COALESCE(users.timezoneid,0) as timezoneid, COALESCE(languages.name,'') as LanguageCode, COALESCE(timezones.name,'') as TimeZoneCode, Password FROM users
...
var GetUserMenusQuery string = `Select M.id as ID, M.name as Name, COALESCE(lc.mediumtext_, shorttext,M.name) as Description,  COALESCE(lnc.name, '') as  LngCode, pagetype as PageType, parentid as ParentID, CASE WHEN pagetype = 3 THEN url ELSE page END as Page, inputs as Inputs, icon as Icon, M.url as Url,
	 CASE WHEN Exists (SELECT 1 FROM menu_roles MR INNER JOIN user_roles UR ON UR.roleid = MR.roleid
					WHERE menuid = M.id AND userid = U.id AND COALESCE(CAST(viewonly AS INTEGER),0) =0) THEN 0
...
WHERE U.id = %d
	AND ( (COALESCE(CAST(mobile AS INTEGER),0) = 1 AND 1 = %d ) OR ( COALESCE(CAST(desktop AS INTEGER),0) = 1  AND 0 = %d) )
	AND COALESCE(parentid,0) = %d
	AND COALESCE(CAST(menushow AS INTEGER),0) = 1
