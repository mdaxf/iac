						...
						ports.push({
							...
							attrs: { 
								circle: { 									
									...
									FunctionInputSource:{
										text: this.data.inputs[i].source == undefined || this.data.inputs[i].source == "1"? '': Function_Source_List[this.data.inputs[i].source] + ' / '+ this.data.inputs[i].aliasname
...
/*
		...
		add_functionInput(block,  name, i,element){
			...
			for(var i=0;i<this.nodes.length;i++){
				if(this.nodes.id == block.data.id){
		...
		add_functionOutput(block,  name, i, element){
			...
			for(var i=0;i<this.nodes.length;i++){
				if(this.nodes.id == block.data.id){
					this.nodes[i].outputs.push(input);
					break;
				}
			}
		...
		build_functionparameter_panel(type,functionid,parameterid){
			...
			*/
			...
			parameterdescription.innerHTML = parameterobj.description;
			...
			parameterdefaultvalue.setAttribute('value',parameterobj.defaultvalue);
			...
			if(type == 'input'){
				...
				parameteraliasname.setAttribute('value',parameterobj.aliasname);
			...
			savebutton.addEventListener('click',function(){
				...
				if(type == 'input'){
					parameterobj.source = parametersource.value;
					parameterobj.aliasname = parameteraliasname.value;
				}
				else{
					...
					$('.parameterdest_selector').each(function(){
						...
						parameterobj.aliasname.push($(this).parent().find('.parameteraliasname').val());
					})
				}
