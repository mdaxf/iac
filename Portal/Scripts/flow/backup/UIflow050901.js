		...
		build_block(){
			...
			switch (this.type.toUpperCase()) {
				...
				case 'OPERATION':
					...
					operation:
					{
						id: inputs.OprSequenceNoList[i],
						OprSequenceNo: inputs.OprSequenceNoList[i],
						WorkCenter: inputs.WorkCenterList[i],
						Description: inputs.DescriptionList[i],
						Elements: elements,
						type: "OPERATION"
					}
					...
					{
						fromnode: 'START',
						tonode: inputs.LinkOprSequenceNoList[i],
						wipcontentclassid: 0,
						reasoncode:'',
						Label: linklabel
					}
				...
				//	console.log(headeredRectangle.getGroupPorts("input"))
					/*
					headeredRectangle.attr('rect/perimeter', function(angle, vertex, direction, distance) {
						// Get the center of the rectangle
						const center = this.getBBox().center();
						// Calculate the radius of the circle that covers the rectangle
						const radius = Math.max(this.getBBox().width, this.getBBox().height) / 2;
						// Calculate the point on the perimeter of the circle that is closest to the target point
						const result = g.rect(center.x - radius, center.y - radius, radius * 2, radius * 2)
						.intersectionWithLineFromCenterToPoint(vertex);
						return result;
				 	 }); */
	...
	//		this.create_events();
		...
		create_events(){
			...
			this.Paper.on('blank:pointerdbclick', function(){
				console.log('blank:pointerdbclick')
				if(that.linkfromelementview){
					that.linkfromelementview.hideTools();
			...
			//	console.log('element:pointerup',that.linkelements, that.linkfromelementview,elementView)
				if(that.linkelements && that.linkfromelementview && that.linkfromelementview != elementView)
				{
					...
					if(fromelement.type =='block' && toelement.type == 'block' ){
							...
							console.log('trigger to add link:',fromelement.obj, toelement.obj)
						...
						if(fromelement.type =='mergepoint' ){
			...
			//	that.linkline.remove();
				that.linkline = null;
				/* track the position change and save it */
				let selectedobj = that.get_object_byelementid(elementView.model.id)
				if(selectedobj.type == 'block'){
					let block = selectedobj.obj;
					let newposition = block.node.shape.position();
				//	console.log(block, newposition)
					if(newposition.x != block.data.x || newposition.y != block.data.y){
						block.data.x = newposition.x;
						block.data.y = newposition.y;
						
						for(var i=0;i<that.nodes.length;i++){
							if (that.nodes[i].id == block.data.id){
								that.nodes[i].x = newposition.x;
								that.nodes[i].y = newposition.y;
								break;
							}						
						}
			...
			this.Paper.on('element:pointerlclick', function(elementView) {
				console.log('element:pointerlclick')
				joint.highlighters.mask.add(elementView, { selector: 'root' }, 'my-element-highlight', {
					deep: true,
					attrs: {
						'stroke': '#FF4365',
