...
(function (UI) {
    ...
    class UserLogin{
        ...
        checkiflogin(success, fail, forcerenew = false){
        ...
        //    // UI.Log(userdata)
            if(userdata){
                ...
                if(checkedtime > new Date() && !forcerenew){
                    if(success){
                        success();
                    }
                    return true;
           ...
           //     // UI.Log(this.token, parsedDate, new Date(), (parsedDate > new Date()))
                ...
                if(parsedDate > new Date() || forcerenew){
                  //  // UI.Log("renew")
                    ...
                    console.log("User token expired, please login again.", fail);
                    this.logout();
...
(function (UI) {    
    ...
    function createFragment(html) {
        ...
        /*translate(html).then((fragment)=>{
            return fragment;
        })   
        let fragment = generateFragment(html)
     //   translate(fragment)
        return fragment;*/
    }
    ...
    async function translate(division){
        ...
        for (let i = 0; i < elements.length; i++) {
            ...
            if(key != null && key != undefined && key != ""){
                ...
                shorts.push(el.innerText); 
            }
        }
        UI.Log("translation:", lngcodes, texts, shorts)
        if(lngcodes.length > 0){
            let inputs = {
                "lngcodes": lngcodes,
                "texts": texts,
                "shorts": shorts,
                "language": UI.userlogin.langauge || "en"
            }
    ...
    */
    ...
    class View extends UI.EventDispatcher{
       ...
       /* async loadconfiguration(configuration){
        ...
        async create(){
          ...
          //  // UI.Log(this, this.Panel.panelElement);
            ...
            if(this.configuration.content){                
                ...
                this.view.innerHTML = await UI.createFragment(this.createcontext(this.content));
        ...
        async buildviewwithresponse(response){
            let that = this;
            ...
            UI.Log(doc, body)
            this.view.appendChild(await UI.createFragment(this.createcontext(body.innerHTML)));
...
function $ViewID_SaveChange(){
    let lngtable = document.querySelector('#$ViewID  ui-tabulator');
    const editedCells = lngtable.Table.getEditedCells();
    let changeddata = [];
    editedCells.forEach((editedCell) => {
      let found = false;
      let celldata = editedCell.getRow().getData();
      UI.Log(celldata);
      for(var i=0;i<changeddata.length;i++){
       if(changeddata[i] == celldata){
         found = true;
         break;
       }
      }
      if(found == false)
        changeddata.push(celldata);       
    });
