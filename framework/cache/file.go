...
func (fc *FileCache) StartAndGC(config string) error {
	//	fmt.Println("file cache config:", config)
	cfg := make(map[string]string)
	err := json.Unmarshal([]byte(config), &cfg)
	if err != nil {
		fmt.Println("file cache config error:", err)
		return err
	}
	//	fmt.Println("file cache config:", cfg)
...
func (fc *FileCache) Init() error {
	...
	//	fmt.Println("file cache path:", fc.CachePath, ok, err)
	if err != nil || ok {
		return err
	}
	err = os.MkdirAll(fc.CachePath, os.ModePerm)
	//	fmt.Println("file cache path:", fc.CachePath, err)
	if err != nil {
...
func (fc *FileCache) getCacheFileName(key string) (string, error) {
	//	fmt.Println("file cache path-getCacheFileName:", fc.CachePath, key)
	m := md5.New()
